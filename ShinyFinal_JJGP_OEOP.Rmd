---
title: "Entrega Final"
author: "Oscar Eduardo Ortega Penagos, José de Jesús García Piña"
date: "`r Sys.Date()`"
...
```{r}
library(tidyverse)
data.base <- read_csv("bd_lupus_shiny_develop2.csv")
```
...
shape_estados <- read_sf(dsn ="/home/domingo/Documentos/Mapas_en_R/conjunto_de_datos",layer = "00ent"
                 #         encoding = "UTF-8"
                 #          use_iconv = TRUE
                         )
class(shape_estados)
...
```{r Librerías para correr nuestra Shiny}
library(shiny)
library(dplyr)
library(ggplot2)
library(viridis)
...
library(shinythemes)
library(bslib)   
# Crear temas para shiny app, Manual https://github.com/rstudio/bslib/
...
ui <- fluidPage(theme = bs_theme(version = 4, bootswatch = "cyborg",bg = "#060606",fg = "#E5F3EC",primary = "#A99A88", base_font = "Serif", code_font = "SFMono_regular", heading_font = "Cursiva"),
  ...
  titlePanel(HTML("<h4 style='font-size: 14px;'>Autores: García Piña José de Jesus, Ortega Penagos Oscar Eduardo</h4>")),
    ...
    sidebarPanel(
      headerPanel(HTML("<h4 style='font-size: 20px;'>Lupus en México: Análisis de Comorbilidades y Distribución Geográfica</h4>")), #titulo del panel
      br(),
...
server <- function(input, output, session) {
  ...
  output$Mapa01<-renderPlot({
    ...
    library(ggrepel)
    # Leemos los contornos de INEGI
    shape_estados <- read_sf(dsn ="/home/domingo/Documentos/Mapas_en_R/conjunto_de_datos",
                             layer = "00ent"
                     #         encoding = "UTF-8"
                     #          use_iconv = TRUE
                             )
